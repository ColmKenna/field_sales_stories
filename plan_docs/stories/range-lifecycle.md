...
- **Ubiquitous language:**
  ...
  - **Remaining** — Run-out Quantity minus quantity on Accepted and Pending orders since the run-out began, in the same unit. A rejected or cancelled order releases its quantity. Shown to reps as an estimate as of their last Sync. Unavailable when Remaining reaches zero.
